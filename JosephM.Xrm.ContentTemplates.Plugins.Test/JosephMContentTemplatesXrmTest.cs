...
{
    [TestClass]
    public class JosephMContentTemplatesXrmTest : XrmTest
    {
        ...
        {
            var Rollups = JosephMContentTemplatesRollupService
            ...
            foreach (var rollup in Rollups)
            {
                var expectedValue1 = JosephMContentTemplatesRollupService.GetRollup(rollup, parent1.Id);
            ...
            {
                Assert.IsNotNull(child2.GetField(rollup.FieldRolledup));
                var expectedValue2 = JosephMContentTemplatesRollupService.GetRollup(rollup, parent1.Id);
            ...
            foreach (var rollup in Rollups)
            {
                var expectedValue2 = JosephMContentTemplatesRollupService.GetRollup(rollup, parent2.Id);
                Assert.IsTrue(XrmEntity.FieldsEqual(expectedValue2, parent2.GetField(rollup.RollupField)));
            }
            //activate and verify updated
        ...
        {
            var i = 0;
            foreach (var rollup in JosephMContentTemplatesRollupService.GetRollupsForRolledupType(contact.LogicalName))
            {
                i++;
